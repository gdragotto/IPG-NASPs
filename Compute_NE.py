...
def IterativeSG(G, max_iter, opt_solver=1, S=[]):
    ...
    Numb_stra_S = [0] * G.m()
    M_pre = [G.m() - 1, 0, deepcopy(Numb_stra_S), deepcopy(S_new), [[]] * G.m(), deepcopy(U_depend), deepcopy(S_new),
             [0, 0], 0]  # strategy S[0] must be in the supp of player m
    # set memory
    ...
    while True and count <= max_iter and time() - time_aux <= 3600:
        ...
        try:
            ne, Profits, S0_j = ComputeNE(M_pre, Back, U_depend, U_p, G.m(), G.n_I(), G.n_C(), Numb_stra, opt_solver,
                                          M_pos[2], M_pos[-1])
        ...
        if ne == []:  # fail to compute such equilibrium
            ...
            Profile = [np.array([sum(ne[int(k + sum(Numb_stra[:p]))] * S[p][k][i] for k in range(Numb_stra[p])) for i in
                                 range(G.n_I()[p] + G.n_C()[p])]) for p in range(G.m())]
            aux_p = 0
    ...
    for S0_j, s0 in enumerate(S0[start:]):
        ...
        for p in range(m):
            ...
            if p != M[0]:
                ...
                # HEURISTIC II
                D.append([candidate for candidate in combinations(
                    HeuristicII(Supp_Stra[p], M[-2][int(sum(M[2][:p])):int(sum(M[2][:p + 1]))], -1, M[-3][p]), s0[p])])
                ...
                D.append([candidate + (M[1],) for candidate in combinations(
                    HeuristicII(Supp_Stra[p], M[-2][int(sum(M[2][:p])):int(sum(M[2][:p + 1]))], M[1], M[-3][p]),
                    s0[p] - 1)])
...
def ComputeNE_NOT_DFS(U_depend, U_p, m, n_I, n_C, Numb_stra, opt_solver, ne_previous, deviator):
    ##### HEURISTIC 6 ####
    size_pre_ne = [sum(1 for j in ne_previous[int(sum(Numb_stra[:p])):int(sum(Numb_stra[:p + 1]))] if j > 10 ** -5) for
                   p in range(deviator)] + [sum(
        1 for j in ne_previous[int(sum(Numb_stra[:deviator])):int(sum(Numb_stra[:deviator + 1]) - 1)] if
        j > 10 ** -5)] + [
                      sum(1 for j in ne_previous[int(sum(Numb_stra[:p])):int(sum(Numb_stra[:p + 1]))] if j > 10 ** -5)
                      for p in range(deviator + 1, m)]
    ...
    for S0_j, s0 in enumerate(S0):
        ...
        for p in range(m):
            ...
            # HEURISTIC II
            if p != deviator:
                D.append([candidate for candidate in combinations(HeuristicII(Numb_stra[p], ne_previous[int(sum(
                    Numb_stra_previous[:p])):int(sum(Numb_stra_previous[:p + 1]))], -1, []), s0[p])])
            else:
                D.append([candidate for candidate in combinations(HeuristicII(Numb_stra[p], ne_previous[int(sum(
                    Numb_stra_previous[:p])):int(sum(Numb_stra_previous[:p + 1]))], -1, [1]), s0[p])])
        ne, Profits = Recursive_Backtracking(m, A_supp, D, 0, U_depend, U_p, opt_solver, Numb_stra)
        if ne != []:  # Nash equilibrium found!
            return ne, Profits
    return [], []
...
def Heuristic6(Supp_Stra, m, size_ne, n_I, n_C):
    ...
    for s0 in product(*[range(1, min(Supp_Stra[p] + 1, n_I[p] + n_C[p] + 2)) for p in range(m)]):
        S0.append(list(s0))
    if m == 2:
        return sorted(S0, key=lambda x: (abs(x[0] - x[1]), max(abs(size_ne[0] - x[0]), abs(size_ne[1] - x[1])),
                                         max(abs(size_ne[0] + 1 - x[0]), abs(size_ne[1] + 1 - x[1])), x[0] + x[1]))
    else:
        return sorted(S0, key=lambda x: (
            max(abs(size_ne[p] - x[p]) for p in range(m)), max(abs(size_ne[p] + 1 - x[p]) for p in range(m)), sum(x),
            max(abs(x[i] - x[j]) for i in range(m) for j in range(i, m))))
...
def Recursive_Backtracking(m, A_supp, D, i, U_depend, U_p, opt_solver, Numb_stra):
    if i == m:  # this is, we have fixed the support for each player
        ...
        while D[i] != []:
            ...
            if D_new != None:
                ne, Profits = Recursive_Backtracking(m, deepcopy(A_supp), deepcopy(D_new), i + 1, U_depend, U_p,
                                                     opt_solver, Numb_stra)
                if ne != []:
                    return ne, Profits
    return [], []
...
def FeasibilityProblem_Gurobi(m, A_supp, U_depend, U_p, Numb_stra, m_p=None):
    ...
    if m_p == None:
        ...
        sigma = [{sp: m_p.addVar(lb=0, vtype="C", name="sigma_" + str(p) + "_" + str(sp)) for sp in A_supp[p]} for p in
                 range(m)]
        m_p.update()
        ...
        for p, S_p in enumerate(Numb_stra):
            for sp in range(S_p):
                if sp in A_supp[p]:
                    m_p.addConstr(U_p[p][sp] + grb.quicksum(
                        sigma[k][sk] * U_depend[p][k][(sp, sk)] for k in range(m) if k != p for sk in A_supp[k]) == v[
                                      p])
                    m_p.update()
                else:
                    m_p.addConstr(U_p[p][sp] + grb.quicksum(
        ...
        for p in range(m):
            for a in set(dp for Ap in D[p] for dp in Ap):  # for all pure strategies in the possible supports
                for a_prime in chain(range(a), range(a + 1, Numb_stra[p])):
                    ...
                    Outcomes_minus_p = [set(d_minus_p for A_minus_p in D[k] for d_minus_p in A_minus_p) for k in
                                        range(p)] + [set([0])] + [
                                           set(d_minus_p for A_minus_p in D[k] for d_minus_p in A_minus_p) for k in
                                           range(p + 1, m)]
                    for s in product(*Outcomes_minus_p):
                        # if a is conditionally dominated by a_prime given D[-p]:
                        if U_p[p][a] + sum(U_depend[p][k][(a, s[k])] for k in range(m) if k != p) >= U_p[p][
                            a_prime] + sum(U_depend[p][k][(a_prime, s[k])] for k in range(m) if k != p):
                            aux = False
                            break
